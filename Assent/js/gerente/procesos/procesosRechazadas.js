function fetchData() {
    return fetch('/jsons/comprasRechazadas.json') // Cambia esto por la ruta a tu archivo JSON
        .then(response => {
            if (!response.ok) {
...
function fillTable(data) {
    ...
    data.forEach(item => {
        ...
        row.innerHTML = `
            ...
            <td>${item.estado}</td>
            <td>
                <button class="view-quote" onclick="viewQuote(${item.id})">Ver Cotización</button>
                <button class="sla" onclick="viewSLA(${item.id})">SLA</button>
            </td>
        `;
...
function viewQuote(id) {
    ...
    window.location.href = `/html/gerente/procesos/cotizaciones/cotizacionesRechazadas.html?id=${id}`;
}
        ...
        if (compra) {
            const docDefinition = {
                content: [
                    ...
                    { text: `Fecha de Entrega: ${compra.fechaRechazo}`, margin: [0, 0, 0, 10] },
                    { text: `Valor Total: ${compra.valor}`, margin: [0, 0, 0, 10] },
                    ...
                    { text: 'Productos', style: 'subheader' },
                    {
                        table: {
                            ...
                            body: [
                                ...
                                ...compra.productos.map(producto => [producto.nombre, '100', producto.fechaEntrega, producto.precio])
                            ]
                        },
                        margin: [0, 10, 0, 10]
                    },
